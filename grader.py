...
def remove_all_makefile():
    for folder in os.listdir():
        if os.path.isdir(folder) and folder.startswith('group'):
...
def run_tests_store_results():
    with open("scores.csv", "a+") as f:
        for folder in os.listdir():
            ...
            if os.path.isdir(folder) and folder.startswith('group') and (open('scores.csv', 'r').read().find(folder) == -1):
                # os.system("cd " + folder)
                output =  subprocess.Popen("cd {folder} && ./run_tests.sh part2".format(folder = folder), shell=True, stdout=subprocess.PIPE)
                ...
                for i in range(0, len(lines)):
                    if lines[i].startswith("Number of passed tests:"):
                        # lines[i + 1] line with number of test cases passed
                            f.write("{folder}, {score}\n".format(folder=folder, score=lines[i + 1]))
                # os.system("cd ..")
